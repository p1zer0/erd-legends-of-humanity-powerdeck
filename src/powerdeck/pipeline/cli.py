"""CLI der Datenschicht – erreichbar über `python3 -m powerdeck deck`."""
...
def main(argv=None):
    ...
    roster, bias_table, state_table = pipeline.load_inputs()
    persons = pipeline.select(roster["persons"], args.only, args.limit)
    if not persons:
    ...
    result = pipeline.build(persons, bias_table, state_table,
